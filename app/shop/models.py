...
class OrderItem(models.Model):
    ...
    def __str__(self):
        """Return the model as a string"""
        return f'{self.email if self.user == None else self.user}\
            ordered {self.product} {self.quantity} times\
                at {self.date_ordered}'


class UserDeliveryInfo(models.Model):
    """User info to help delivering items"""
    # User is optional but email isn't
    # Email ties the order to a point of contact
    # that email can be used to query the database to find order
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        blank=True,
    ...
    def __str__(self):
        """Return the model as a string"""
        return f'{self.email if self.user == None else self.user}\'s\
            User Delivery Info'
...
class Order(models.Model):
    ...
    def __str__(self):
        """Return the model as a string"""
        return f'{self.email if self.user == None else self.user}\'s \
            order posted at {self.date_ordered}\
                with delivery status of {self.delivery_status}'
